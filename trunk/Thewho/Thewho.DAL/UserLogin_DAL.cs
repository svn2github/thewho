...
namespace Thewho.DAL
{
    ...
    public class UserLogin_DAL
    {
        ...
        //SQL语句 - 分页
        private const string _SQL_SELECT_PAGING = "SELECT [ID],[UserID],[Email],[LoginTime],[HostName],[HostAddress],[Remark],[Result] FROM (SELECT ROW_NUMBER() OVER(ORDER BY @OrderID @OrderType) AS ROWNUM,[ID],[UserID],[Email],[LoginTime],[HostName],[HostAddress],[Remark],[Result] FROM UserLogin {0}) AS T WHERE ROWNUM BETWEEN (@PageIndex-1) * @PageSize + 1 AND (@PageIndex * @PageSize)"; 
        ...
        /// <returns></returns>
        private List<UserLogin> Paging(int pageIndex, int pageSize, string orderID, string orderType, string whereStr, SqlParameter[] whereParms, out int recordCount)
        {
            //将WHERE条件组合进SQL语句
