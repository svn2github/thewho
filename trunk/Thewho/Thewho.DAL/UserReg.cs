...
namespace Thewho.DAL
{
    ...
    public class UserReg_DAL
    {
        ...
        //SQL语句
        private const string _SQL_INSERT = "INSERT INTO UserReg [Email],[Password],[Salt] VALUES(@Email,@Password,@Salt) ";        private const string _SQL_DELETE = "DELETE FROM UserReg WHERE [UID] = @UID";        private const string _SQL_UPDATE = "UPDATE UserReg SET [Email] = @Email,[Password] = @Password,[Salt] = @Salt WHERE [UID] = @UID";        private const string _SQL_SELECT = "SELECT UserReg SET [Email],[Password],[Salt] FROM UserReg";
        #endregion
