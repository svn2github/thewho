...
namespace Thewho.Common
{
    ...
    public sealed class SqlHelper2
    {
        ...
        /// <param>标识数据库连接对象是由调用者提供还是由SqlHelper2提供</param>
        ...
        /// <summary>
        /// 通用分页方法 (精简改进版,这个分页是没做注入之类的判断的,所以请在外部过滤掉再用这个方法) 编写：Tao
        /// </summary>
        ...
        public static SqlDataReader Paging
            (string connectionString, int PageIndex, int PageSize, string OrderID, string OrderType,string StrWhere, string TableName, out int RecordCount)
        {
            RecordCount = 0;
            //RecordCount = PagCount(connectionString, TableName, OrderID, StrWhere);
            ...
            string p = @"
            SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY @OrderID @OrderType) AS ROWNUM,* FROM WHERE 1=1 ) 
            AS T WHERE ROWNUM BETWEEN (@PAGEINDEX-1) * @PAGESIZE + 1 AND (@PAGEINDEX * @PAGESIZE)";

            return SqlHelper2.ExecuteReader(connectionString, CommandType.Text, sql, _parms);
        }
        ...
        /// <returns></returns>
        public static int PagCount(string connectionString, string TableName, string KeyID, string StrWhere)
        {
            ...
            return Convert.ToInt32(SqlHelper2.ExecuteScalar(connectionString, CommandType.Text, sql));
        }


        public static string Between(string columnName, object a, object b)
        {
            return columnName + "BETWEEN " + a + " AND " + b;
        }
        public static string MoreThan(string columnName, object a)
        {
            return columnName + " > " + a;
        }
        public static string LessThan(string columnName, object a)
        {
            return columnName + " < " + a;
        }
        public static string Equal(string columnName, object a)
        {
            return columnName + " = " + a;
        }
        public static string NotEqual(string columnName, object a)
        {
            return columnName + " != " + a;
        }
        public static string Like(string columnName, object a)
        {
            return columnName + " LIKE " + a;
        }
        public static string In(string columnName, object a)
        {
            return columnName + " IN (" + a + ")";
        }
        public static string NotIn(string columnName, object a)
        {
            return columnName + " NOT IN (" + a + ")";
        }
